(async () => {
  ...
  let percentage = 50,
    money = macro_actor.data.data.currency.cp;
  //if gm ask for %
  if (game.user.isGM)
    percentage = parseInt(
      await choose(`percent`, percentage, `Percentage of value available : `)
    );
  display();
  ...
  function display() {
    ...
    inventory = items.map((item) => {
      return {
        ...
        id: item.id,
        name: item.name,
        price: Math.floor((item.data.data.price * percentage) / 100),
        ...
        Ok: {
          ...
          callback: async (html) => {
            ...
            inventory.forEach((item) => {
              overall_value += item.sold * item.price;
            });
            await macro_actor.update({
              "data.currency.gp": money + Math.floor(overall_value),
            });
            ...
            await macro_actor.deleteOwnedItem(
              update_items
                .filter((item) => item.data.data.quantity === 0)
                .map((item) => item.id)
            );
            await macro_actor.updateOwnedItem(
              update_items
                .filter((item) => item.data.data.quantity !== 0)
                .map((item) => {
                  return { _id: item.id, data: item.data };
                })
            );
            ...
            ChatMessage.create({
              ...
              content: `
                ...
                <tr style="border-bottom:1px solid #000">
                  <th style="width=60%"><img src="${
                    macro_actor.data.token.img
    ...
    function update_data() {
      ...
      for (let item of inventory) {
        ...
        if (element.value !== undefined && element.value !== item.sold) {
          ...
          let overallvalue = 0;
          inventory.forEach((item) => {
            overallvalue += item.sold * item.price;
          });
          display_dialog.data.buttons.Ok.label = `${Math.floor(
            overallvalue
          )} GP`;
