...
const config = {
  merchants: {
    object: game.actors.filter(
      (a) =>
        a.data.type === `npc` &&
        !!a.data.effects.find((e) => e.label === `Merchant` && !e.disabled)
    ),
    htmlID: randomID(),
    buttonIds: [],
  },
  ...
  fn: {
    log: (...args) =>
      args.forEach((m) => console.log(`${this.name} Macro | `, m)),
...
async function main() {
  ...
  function getContent(html) {
    const [merchant, purchaser] = html || [
      config.merchants.object[0].id,
      config.purchasers.object[0].id,
    ];
    ...
    function getDropDown(object = ``, selected) {
      return `
        <select name="${object}" id="${config[object].htmlID}" ${
        config[object].object.length === 1 ? `disabled` : ``
      }>
          ${config[object].object.reduce(
            (a, v, i, r) =>
              (a += `<option value="${v.id}" ${
    ...
    function getCurrency(object = ``, selected) {
      let currencyData = config[object].object.find((a, i) =>
        !!selected ? a.id === selected : i === 0
      ).data.data.currency;
      return Object.entries(currencyData).reduce(
        (acc, [key, value]) =>
    ...
    function getItemList(object = ``, selected) {
      ...
      let itemData = selectedActor.items
        .filter((i) => i.data.data.price !== 0 && !!i.data.data.price)
        .map((i) => ({
          _id: i.id,
          actorId: i.actor.id,
          type: object,
      ...
      config[object].buttonIds = itemData.map(
        (i) => `${i.actorId}.${i._id}.${i.type}`
      );
      ...
      let percent =
        object === `merchants`
          ...
          : new Roll(
              config.purchasers.percent,
              selectedActor.getRollData()
            ).roll().total;
      ...
      return `
        <table style="width:100%">
          ...
          ${itemData.reduce(
            (acc, { _id, name, quantity, price, img, actorId, type }) =>
              (acc += `
                <tr>
                  ...
                  <td style="text-align:center;"><button id="${actorId}.${_id}.${type}" value="${actorId}.${_id}.${type}" type="button" ${
                quantity === 0 ? `disabled` : ``
              }>${object === `merchants` ? `Buy` : `Sell`}</button></td>
                </tr>
              `),
            ``
          )}
  ...
  function getValues() {
    let html = document
      .getElementById(`app-${config.appID}`)
  ...
  async function transaction(executionString) {
    const [merchant, purchaser] = getValues() || [
      config.merchants.object.length === 1
        ? config.merchants.object[0].id
        : undefined,
      config.purchasers.object.length === 1
        ? config.purchasers.object[0].id
        : undefined,
    ];
    ...
    let percent =
      type === `purchasers`
        ? new Roll(config.purchasers.percent, from.getRollData()).roll().total
        : parseInt(from.getFlag(`world`, `merchant-percentage`)) || 100;
    let value = item.data.data.price;
    let to =
      actorID === merchant
        ? game.actors.get(purchaser)
        : game.actors.get(merchant);
    ...
    if (getFunds(to) < actualValue) {
      ui.notifications.warn(
        `${to.name} has insufficent funds to purchase ${item.name} at the value of ${actualValue} gp.`
      );
    } else {
    ...
    async function updateFunds(actor, value) {
      let base = value * 100;
      let pp = (base - (base % 1000)) / 1000;
      base = base - pp * 1000;
      let gp = (base - (base % 100)) / 100;
      base = base - gp * 100;
      let ep = (base - (base % 50)) / 50;
      base = base - ep * 50;
      let sp = (base - (base % 10)) / 10;
      base = base - sp * 10;
...
async function main() {
  ...
  function getHTML() {
    ...
    Object.entries(config.eventIDs).forEach(([key, value]) => {
      ...
      if (value instanceof Array) {
        for (let v of value) data[v] = document.getElementById(v)?.checked;
      } else {
  ...
  function getContent(html) {
    ...
    function getMerchants() {
      return `
        <select name="merchants" id="${config.eventIDs.merchant}" ${
        config.merchants.object.length === 1 ? `disabled` : ``
      }>
          ${config.merchants.object.reduce(
            (a, v, i, r) =>
              (a += `<option value="${v.id}" ${
                selectedMerchant == v.id ? `selected` : ``
              }>${v.name}</option>`),
            ``
          )}
        </select>`;
      ...
      return `
          <table style="">
            <tr>
              ...
              <td><input type="number" id="${config.eventIDs.percent}" value="${
        percent || 100
      }"></td>
            </tr>
            <tr>
              <td>PP</td>
              <td><input type="number" id="${config.eventIDs.pp}" value="${
        currency?.pp || 0
      }"></td>
            </tr>
            <tr>
              <td>GP</td>
              <td><input type="number" id="${config.eventIDs.gp}" value="${
        currency?.gp || 0
      }"></td>
            </tr>
            <tr>
              <td>EP</td>
              <td><input type="number" id="${config.eventIDs.ep}" value="${
        currency?.ep || 0
      }"></td>
            </tr>
            <tr>
              <td>SP</td>
              <td><input type="number" id="${config.eventIDs.sp}" value="${
        currency?.sp || 0
      }"></td>
            </tr>
            <tr>
              <td>CP</td>
              <td><input type="number" id="${config.eventIDs.cp}" value="${
        currency?.cp || 0
      }"></td>
            </tr>
          </table>
    ...
    function getPermission() {
      ...
      return `
          ${config.users.object.reduce(
            (acc, val, ind, arr) =>
              (acc += `
            <div style="display:flex;width:100%;height:40px;justify-content:space-between;">
              <label>${val.name}</label><input type="checkbox" id="user.${
                val.id
              }" ${gpa.data.permission[val.id] === 3 ? `checked` : ``}> 
            </div>
          `),
            ``
          )}
